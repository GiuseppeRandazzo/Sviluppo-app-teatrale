...
const Alert = ({
  ...
  // Configurazioni per i diversi tipi di alert
  const alertConfig = {
    info: {
      bgColor: 'bg-blue-50',
  ...
  return (
    <div
      className={`p-4 mb-4 border-l-4 ${config.bgColor} ${config.borderColor} ${config.textColor} rounded-r-md ${className}`}
      role="alert"
      {...props}
    >
      <div className="flex items-start">
        {icon && <div className="flex-shrink-0 mr-3">{config.icon}</div>}
        ...
        {dismissible && onDismiss && (
          <button
            type="button"
            className={`ml-auto -mx-1.5 -my-1.5 ${config.bgColor} ${config.textColor} rounded-lg focus:ring-2 focus:ring-${type}-400 p-1.5 inline-flex h-8 w-8`}
            onClick={onDismiss}
            aria-label="Chiudi"
          >
